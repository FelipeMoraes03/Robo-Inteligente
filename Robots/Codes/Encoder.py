...
class Encoder(multiprocessing.Process):
    def __init__(self, pinA, pinB, bt = 8) -> None:
        super().__init__()
        self.ENCA = pinA
        self.ENCB = pinB
    ...
    def run(self):
        GPIO.add_event_detect(self.ENCA, GPIO.RISING, callback=self._angle_callback, bouncetime=self.bt)
        GPIO.add_event_detect(self.ENCB, GPIO.RISING, callback=self._angle_callback, bouncetime=self.bt)
